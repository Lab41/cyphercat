...
from .metrics import eval_membership_inference, eval_attack_net


def ml_leaks1(target=None, shadow_net=None, attacker_net=None,
              target_train_loader=None, target_out_loader=None,
              shadow_train_loader=None, shadow_out_loader=None,
              test_loader=None, shadow_optim=None, attack_optim=None,
              shadow_criterion=None, attack_criterion=None, n_epochs=0,
              classes=None, n_max_posteriors=3, verbose=False):
    ...
    Args:
        ...
        shadow_net (nn.Module): Shadow network to help train the attacker in
            membership inference task.
        attacker_net (nn.Module): Network to be trained in membership inference
            task.
        ...
        test_loader: Loader pointing to the validation data set, i.e. out of
         ...
         shadow_optim (torch.optim): Optimizer for shadow_net training.
         attack_optim (torch.optim): Optimizer for attacker_net training.
         shadow_criterion (torch.nn): Loss function for shadow_net training.
         attack_criterion (torch.nn): Loss function for attacker_net training.
    ...
    train(net=shadow_net, data_loader=shadow_train_loader,
          test_loader=test_loader, optimizer=shadow_optim,
          criterion=shadow_criterion, n_epochs=n_epochs, classes=classes,
          verbose=verbose)
    #
    print('---- Training attack network ----')
    train_attacker(attack_net=attacker_net, shadow=shadow_net,
...
def ml_leaks3(target=None, target_train_loader=None,  target_out_loader=None):
    ...
    To-do:
         Add example to docstring.
    '''
    eval_membership_inference(target_net=target,
                              target_train=target_train_loader,
